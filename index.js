...
function createMenuItem(item, cost, service){
  const newItem = {name : item, price: cost, category: service};
  return newItem;
}
...
const spanakopita = createMenuItem("Spanakopita", 15, 'Breakfast');
const hotChocolate = createMenuItem("Hot Chocolate", 8, 'Drinks');
...
function getReviewByIndex(anArray, index) {
  return `${anArray[index].name} gave the restaurant a ${anArray[index].rating} star review, and their feedback was: ${anArray[index].feedback}`
}
...
function getLastReview(anArray) {
 let index = anArray.length - 1;
  return `${anArray[index].name} gave the restaurant a ${anArray[index].rating} star review, and their feedback was: ${anArray[index].feedback}`;
} 
